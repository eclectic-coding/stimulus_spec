...
RSpec.describe StimulusSpec::Matchers::HaveStimulusOutlet do
  ...
  describe "#within" do
    it "matches within the given selector" do
      html = '<form class="search"><div data-search-results-outlet="#results-list"></div></form>'
      expect(html).to have_stimulus_outlet("search", "results").within(".search")
    end

    it "does not match outside the given selector" do
      html = '<div data-search-results-outlet="#results-list"></div><form class="search"></form>'
      expect(html).not_to have_stimulus_outlet("search", "results").within(".search")
    end

    it "returns false when scope element is not found" do
      html = '<div data-search-results-outlet="#results-list"></div>'
      expect(html).not_to have_stimulus_outlet("search", "results").within(".missing")
    end
  end
